...
async def search(action: actionengine.Action):
    input_timeout = 30.0
    ...
    async with httpx.AsyncClient(base_url=api_url) as ac:
        async with ac.stream(
            ...
            async for status_or_memory in api_utils.iterate_ndjson(
                response, [ObjectMemoryOut, Status]
            ):
                if isinstance(status_or_memory, Status):
                    raise status_or_memory.to_exception()

                status_or_memory: ObjectMemoryOut
                await action["memories"].put(status_or_memory)

            await action["memories"].finalize()
